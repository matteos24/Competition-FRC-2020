...
public class SlalomCommandAuto extends SequentialCommandGroup {
    public SlalomCommandAuto(Drivetrain drivetrain, Vision vision) {
        super();
        ...
        //new SwerveCommand(drivetrain, leftSide, rightSide, Speed);
        new MoveCommand(drivetrain, sqrt(4212),0.8); // Moving below D10
        for (int i = 0; i < 2; i++) { // iteration loop to move around last obstavle
